...
4. Read each affected page's `sources` directly. Do not rely on the wiki summary alone for implementation details.
   - After a work item is selected and prospective PR metadata exists, a committed exact revision may emit one reusable, body-free handoff with `bun run wiki:context -- --work <ID> --artifact <path> --metadata <pr-body> --base <ref>`. A later role must validate it with the same command using `--reuse <path>` before relying on its routing data. Any work selector, metadata, base, merge-base, HEAD, controlling page, conflict, source, or read-order change invalidates the whole artifact. Reuse never replaces reading the listed current pages and sources directly.
