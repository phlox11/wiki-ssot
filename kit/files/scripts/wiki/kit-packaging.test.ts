...
function tempDir(prefix = "kit-packaging-test-"): string {
...
describe("kit entry table", () => {
  ...
  test("keeps the publisher-only suite out of shipped entries", () => {
    expect(KIT_ENTRIES.some((entry) => entry.target === "scripts/wiki/kit-packaging-publisher.test.ts")).toBe(false);
  });
...
describe("emitted kit", () => {
  ...
  test("ships each KM-02 and KM-03 module and focused suite exactly once", () => {
    const { files } = realKit();
    const targets = [
