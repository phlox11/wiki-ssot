...
Proposal pages may own `work_items` in structured frontmatter. The engine validates their repository-wide ID graph, lifecycle fields, optional `agent | human | either` executor, current-page context targets, dependencies, priorities, blockers, deferred reasons, and durable done evidence. Executor is independent from state and omission normalizes to `agent`. It derives `not-started` into `ready` or `waiting` over the complete graph, then applies executor views without changing dependency meaning. Recommendations consider only `agent` or `either`, prefer active work before ready work by explicit priority and ID, and never auto-select human-exclusive, blocked, waiting, deferred, or conflict records.

`wiki:work` needs no search term and works offline. Its default and all-executor views keep human work visible, while the human view supports reporting and handoff without agent execution or assumed credentials. Executor classification, including `either`, grants no authority. Its deterministic Markdown twin, `wiki/work-queue.md`, is generated and linked from both entrypoints. `wiki:context -- --work <ID>` then assembles a compact routing projection of the work contract, executor guidance, every current invariant, declared current context pages, related open conflicts, exact/glob sources, and the owning proposal under the explicit label `NON-CURRENT WORK OWNER`. It retains status, authority, paths, source expansion, conflicts, read order, stable body digests, and focused commands without embedding every body; `--full` restores the exhaustive representation. This makes a plain "what work remains?" sufficient for re-entry without turning proposals into current authority or making the engine interpret natural language.

Topic context uses the same deterministic search operation. Complete all-term matches still select full semantic context, projected compactly by default. When no page contains every term, the default response is an ordered candidate projection rather than an expansion of every partial-match body. Each candidate keeps its score, authority/status, path, digest, and focused command, so a caller can choose the next layer without losing reachability; `--full` remains the explicit compatibility path for exhaustive historical output.
