...
The focused-review implementation is bound to `113fc75e338fd7db42303a2eb178a91cd26e8e3f`. The paired historical control remains the TE-00 publisher case at `6fd3a85414e00892930557cb8335e2d88ec90d66`; both controlled rollouts used one `gpt-5.6-sol / high` default agent.
...
| Model calls | 8 | 2 | -6 |
| Reviewer active time | 84,600 ms | 14,172 ms | -70,428 ms |
...
The controlled after-case used two model calls, 50,932 raw input tokens including 48,640 cached and 2,292 derived uncached tokens, 355 output tokens including 110 reasoning tokens, one tool call, zero compactions, 14,172 ms of active task time, and 14,172 ms wall time. The earlier superseded pilots are disclosed in the JSON limitations but are not substituted for the final exact-revision measurement.
