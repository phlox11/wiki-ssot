...
The focused-review implementation is bound to `fa332b19f19a5f9333fe64e3bd2d80c0e766885f`. The paired historical control remains the TE-00 publisher case at `6fd3a85414e00892930557cb8335e2d88ec90d66`; both controlled rollouts used one `gpt-5.6-sol / high` default agent.
...
| Model calls | 8 | 2 | -6 |
| Reviewer active time | 84,600 ms | 16,970 ms | -67,630 ms |
...
The controlled after-case used two model calls, 51,054 raw input tokens including 31,232 cached and 19,822 derived uncached tokens, 365 output tokens including 120 reasoning tokens, one tool call, zero compactions, 16,970 ms of active task time, and 16,970 ms wall time. The earlier superseded pilot and its digest-label correction are disclosed in the JSON limitations but are not substituted for the final exact-revision measurement.
