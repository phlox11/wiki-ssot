...
The provider-neutral engine has an explicit shared foundation. `scripts/wiki/model.ts` owns Wiki, source, conflict, work, page, and finding models; `scripts/wiki/serialization.ts` owns stable JSON and content hashing; `scripts/wiki/repository-view.ts` owns normalized repository paths plus working-tree or staged Git-backed reads; and `scripts/wiki/page-validation.ts` and `scripts/wiki/work-validation.ts` own page/link/source and work-graph validation. `scripts/wiki/discovery.ts` derives current pages, conflicts, search results, work queues, executor projections, and recommendations above that foundation. `scripts/wiki/context.ts` owns selected-work, topic, exact-page, compact/full projection, source expansion, and authoritative read-order models, while `scripts/wiki/generated-views.ts` renders the deterministic index, current status, work queue, conflict index, and source/conflict maps. These modules point only toward lower foundation or discovery layers, and none imports `scripts/wiki/core.ts`.

`core.ts` remains the compatibility facade for existing shipped and publishing-only callers while continuing to own reusable selected-work artifact bindings, generated-file comparison and writing, kit packaging, coverage and impact, PR metadata and conflict lifecycle, deterministic risk classification, review manifests, and Fresh-context report validation over version 1 free-text findings or version 2 structured findings. The CLI continues importing through that facade until the bounded-handler stage, so the frozen command, text, JSON, stdout/stderr, and usage contracts do not change during this split.
