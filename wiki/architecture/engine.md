...
Version 2 findings are adjudicated, not merely shape-checked. A fixed table decides which dispositions retire which classification, so `candidate_regression` and `declared_contract_violation` accept only `fixed` or `unresolved`, `decision_ambiguity` is retired only into a conflict, and `recorded` is confined to a `suggestion`. A disposition naming a conflict is resolved against the open conflicts at the reviewed HEAD and must match on conflict type, on `origin: baseline` for a classification asserting the problem predates the candidate, and on at least one page shared with the finding's `page:` scope refs. Because `origin` is authored and `classification` is reviewed independently, that agreement check is what keeps `conflict-introduced-high-risk` load-bearing. `validateFreshContextFindings` still runs shape-only when no conflict list is supplied; `reviewCheck` always supplies one, so an empty repository conflict set makes every pointer dangling.
