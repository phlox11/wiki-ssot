...
The engine has one-way layers: `model.ts` owns Wiki models; `serialization.ts` stable JSON and hashes; `repository-view.ts` normalized working or staged reads; page/work validation their respective rules; `discovery.ts` search and queues; `context.ts` source-complete projections; `generated-views.ts` deterministic Wiki views and maps; and `kit-packaging.ts` portable entries, rendering, manifests, comparison, and writes. None imports `core.ts`.

`core.ts` stays the compatibility facade and owns handoffs, non-kit generation, impact/coverage, PR/conflict lifecycle, and review/attestation. The unchanged CLI still imports it, preserving command, text, JSON, stdout/stderr, and usage contracts until the bounded-handler stage. `kit-sync.ts` remains standalone for pre-install use.
...
`scripts/wiki/wiki.test.ts`, `scripts/wiki/fresh-context.test.ts`, `scripts/wiki/work.test.ts`, `scripts/wiki/kit-packaging.test.ts`, `scripts/wiki/kit.test.ts`, and `scripts/wiki/apply.test.ts` exercise the engine against synthetic in-memory and temporary git repositories. The packaging suite owns entry, exclusion, rendering, manifest, comparison, and write contracts; the remaining kit suite owns integration and sync semantics. Together with the other suites they cover work behavior, determinism, review validation, risk selection, trust, CLI behavior, all three apply lifecycles, dry-run and idempotence, managed/package ownership, version 1 migration, customized-file safety, and the install → semantic verification → `ready` loop. `scripts/wiki/test-runner.ts` discovers the Wiki tests and runs each in an isolated Bun process with bounded parallelism, preventing adoption fixtures and historical evaluators from sharing accumulated memory while limiting downstream execution to shipped tests. Older adoption fixtures remain reproducibility evidence for the former lower-level path. These tests depend only on `bun`, `typescript`, and `yaml`; git-backed reads shell out to `git`.
