...
Provider-neutral logic lives in `scripts/wiki/core.ts`: frontmatter/schema/link checks, source→page mapping, coverage, blob-hash staleness (`.wiki/state.json`), diff impact, PR metadata and conflict lifecycle, provider-neutral integration-seam validation, deterministic risk classification, review manifests, and Fresh-context report validation. `scripts/wiki/cli.ts` is a thin command layer exposing pre-PR `wiki:review-preflight`, lower-level `wiki:review-bundle` / `wiki:review-check`, and `wiki:doctor` with stable machine-readable findings. Preflight returns `not-required`, `review-required`, `needs-reconcile`, or `pass`, prepares the exact bundle before a PR exists, and validates a separate review context's report without treating the still-pending PR-body mirror as evidence.
...
The version 1 bundle's prompt, report contract, and impact JSON shape remain byte-compatible across engine upgrades so a report prepared before publication is accepted by the merge-base-pinned Ready CI validator. New authoring guidance can become stricter without silently changing those bound v1 files; a future bundle format change requires an explicit versioned transition.
