...
`kit-packaging.test.ts` ships adopter-safe coverage; publisher-only checks stay in unshipped `kit-packaging-publisher.test.ts`. `kit.test.ts` covers integration and sync. The other synthetic or Git-backed suites cover work/context, review/impact/trust, CLI behavior, and new/adopt/upgrade flows including dry-run, idempotence, managed/package ownership, version 1 migration, customization safety, and the final `ready` loop. `test-runner.ts` runs each shipped Wiki test in an isolated bounded-parallel Bun process so adoption and historical fixtures do not share accumulated memory. Older adoption fixtures remain reproducibility evidence. The suites require only `bun`, `typescript`, `yaml`, and `git`.
