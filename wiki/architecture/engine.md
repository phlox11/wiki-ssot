...
Placement decides what happens downstream. `kit/files/**` is kit-owned and replaced on upgrade; `kit/seed/**` is written only when absent and never updated, so adopter policy, recorded source hashes, and a project-specific inventory implementation survive; a `reference` file such as `kit/package.kit.json` is read from the kit checkout and never copied, so nothing an adopter merges away is re-created by the next upgrade. `kit/files/.wiki/kit-manifest.json` carries that split plus a sha256 per file and a content-addressed roll-up digest — no version tag or timestamp, so the same sources always produce the same manifest.

`kit:exclude` regions keep instance-only guidance out of the payload: rules about developing this engine and links to pages only this repository has are stripped from `AGENTS.md` and `wiki/WORKFLOW.md`, while the `wiki-ssot:fresh-context-guardrail` marker the seam check requires is preserved. An unbalanced or unclosed marker fails generation rather than truncating a file. Because `kit/**` is a derived copy of files that already carry the wiki obligation, it is excluded from implementation-source classification and from the Markdown link check, whose relative targets resolve at the adopting repository's root rather than here.

`scripts/wiki/kit-sync.ts` is the standalone adoption and upgrade tool, run from a wiki-ssot checkout against another repository. It imports nothing from the engine because it must run against a repository that has not installed it yet. For each kit-owned file it compares the incoming version, the version recorded in the target's manifest, and the version on disk, yielding `create`, `unchanged`, `update`, `customized`, or `conflict`; an existing file with no recorded manifest is a conflict because it cannot be proven pristine. A conflict writes the incoming bytes to `<path>.kit-new`, never overwrites, and withholds the manifest advance so an unresolved conflict cannot be downgraded into a customization by the next run.
