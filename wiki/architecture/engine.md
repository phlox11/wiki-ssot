...
Provider-neutral logic lives in `scripts/wiki/core.ts`: frontmatter/schema/link checks, source→page mapping, coverage, blob-hash staleness (`.wiki/state.json`), diff impact, PR metadata and conflict lifecycle, provider-neutral integration-seam validation, deterministic review manifests, and Fresh-context report validation. `scripts/wiki/cli.ts` is a thin command layer exposing `wiki:review-bundle`, `wiki:review-check`, and `wiki:doctor` with stable machine-readable findings. `wiki:doctor` composes the core seam checks with the selected provider adapter's checks.
