...
The provider-neutral engine is layered. `model.ts` owns Wiki models, `serialization.ts` stable JSON and hashes, `repository-view.ts` normalized working-tree or staged reads, and `page-validation.ts` plus `work-validation.ts` validation. Above them, `discovery.ts` owns page/conflict search and work queues, `context.ts` source-complete context and projections, and `generated-views.ts` deterministic Wiki views and maps. Dependencies point only downward and none of these modules imports `core.ts`.

`core.ts` remains the compatibility facade and owns reusable handoffs, generated-file comparison/writes, kit packaging, impact and coverage, PR/conflict lifecycle, review manifests, and Fresh-context validation. The unchanged CLI still imports through it, preserving command, text, JSON, stdout/stderr, and usage contracts until the bounded-handler stage.
