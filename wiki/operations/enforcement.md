...
- **Pull request (CI).** `.github/workflows/wiki-ssot.yml` runs only the structural Wiki jobs on pull requests and runs `wiki-review-attestation` only for non-Draft PRs; host build/test jobs stay in host workflows such as this publisher's `checks.yml`. Required reports are attached to a Draft after local PASS, mirrored into the PR body, and then validated when the PR becomes Ready. The check name deliberately describes validation of an existing attestation; it does not perform semantic Fresh-context review in CI.
