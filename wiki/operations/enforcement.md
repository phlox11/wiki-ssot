...
- **Pre-PR semantic reconciliation.** After deterministic checks and prospective PR metadata are complete, `wiki:review-preflight` evaluates trusted risk policy before any PR exists. Low-risk candidates are ready immediately. For selected changes it emits an exact bundle for a context-isolated reviewer; the authoring agent reconciles actionable `NEEDS_RECONCILE` findings and reruns preflight on the new HEAD until the candidate passes or requires an explicit owner decision.
- **Pull request (CI).** `.github/workflows/checks.yml` runs the structural jobs on pull requests and runs `wiki-fresh-context` only for non-Draft PRs. Required reports are attached to a Draft after local PASS, mirrored into the PR body, and then validated when the PR becomes Ready.
