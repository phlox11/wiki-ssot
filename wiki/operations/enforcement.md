...
summary: Three rails enforce the wiki — zero-knowledge agent entry, local hooks, and blocking CI including the trusted wiki-review-attestation check, with branch protection as the durable merge boundary.
kind: operation
status: current
authority: normative
owners: ["@phlox11"]
...
`wiki:doctor` treats work discovery as its own installation seam. It fails with stable findings when either the root `wiki-ssot:work-discovery` marker/generic-request guidance or canonical `wiki:work` package script disappears. This checks that a fresh session can reach the deterministic command; it does not claim the engine classifies natural language or that an agent followed the result.
