...
`wiki:doctor` treats the root agent entrypoint as a provider-neutral installation seam. In addition to the integration markers and canonical package scripts, the entrypoint must contain meaningful line-level routes from session start to the wiki index, current status, and invariants; from generic remaining-work intent to no-query `wiki:work`; from a returned selection to `wiki:context -- --work <ID>`; and from topic work to search/context, while explicitly keeping proposed, conflicted, deprecated, and archived pages non-current. Marker-only, marker-plus-command placeholders, and command-name-only lists fail deterministically. This validates an installed routing contract, not natural-language classification or proof that an agent followed it.
