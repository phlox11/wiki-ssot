...
- **Remaining work is discoverable without prior node knowledge.** Proposal work records form one validated repository-wide graph with unique IDs, current-page context targets, legal lifecycle fields, acyclic known dependencies, blockers/reasons, and durable done evidence. `wiki:work` requires no query, derives ready/waiting state, includes open conflicts, and recommends only active or ready work by explicit priority and ID. The provider-neutral agent entrypoint must meaningfully route session start through the wiki index, current status, and invariants; generic remaining-work intent through the no-query queue; returned selections through focused work context; and topic work through search/context without presenting non-current pages as current authority. A selected item exposes its work contract, all current invariants, current context pages, related conflicts, exact source declarations, deterministic glob matches, and a clearly non-current proposal owner. Text and JSON carry the same authority and source fields, and the authoritative read order is invariant → conflict → current page → expanded source.
