...
bun run typecheck
bun run test
bun run wiki:review-check -- --base origin/main --metadata /path/to/pr-body.md --json
```

Fill the PR metadata block, including the structured `fresh_context` status mirror, before evaluating the trusted risk policy. `WIKI_PR_BODY` may provide the same block when a file is inconvenient. A low-risk result reports `required: false` and needs no bundle or external report.

When `required: true`, create the bundle with `bun run wiki:review-bundle -- --base origin/main --metadata /path/to/pr-body.md`. A separate context-isolated session or reviewer—not the authoring session—reads the bundle and primary sources, then creates the version 1 report described by `REPORT.md`. Publish it through the trusted PR review/comment channel, mirror its verdict/HEAD/bundle/reviewer/evidence into the PR body's `fresh_context` block, and validate it:
...
When required, the report must be PASS with evidence and must bind the current full HEAD, merge-base, and bundle digest. `NEEDS_RECONCILE` means fix → new bundle → new review. Any new commit or semantic PR metadata change makes the old PASS stale. If a required separate reviewer/report is unavailable, leave the PR Draft and ask the user for the reviewer or permission; do not report completion or mark it Ready.

`requireDifferentActor: false` permits a solo maintainer's authenticated GitHub account to publish a required separate session's report; it does not permit the authoring session to invent its own PASS. `true` additionally requires the publisher to differ from the PR author and must only be enabled after a second account or bot is available.

## Enforcement layers

- pre-commit: staged wiki structure/link/source/generated validation only.
- pre-push: direct `main` push prevention.
- CI: code tests, wiki structure/doctor, generated freshness, blocking impact enforcement, and the `wiki-fresh-context` policy/attestation check, plus a weekly full audit. Ready/merge requires a trusted non-required classification or a current PASS.
