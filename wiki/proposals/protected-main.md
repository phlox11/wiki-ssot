...
- **Require status checks:** `code-check`, `wiki-structure`, `wiki-generated`, and `wiki-impact`. A check only appears in the list after it has run once.
...
This page stays `status: proposed` until a repository shows the ruleset active. Organization-owned private repositories require a plan that enforces rulesets (for example GitHub Team); until then `main` is protected only by the honor system and the local hooks described in [operations/enforcement](../operations/enforcement.md).
