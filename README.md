...
wiki-ssot fixes both with **deterministic gates** — pure functions of your tracked files, with no LLM in the blocking path — wired onto the events that always happen: session start, commit, and pull request.
...
- Everything blocking is **deterministic**. The one place an LLM helps — a fresh-context "does this PR keep the wiki coherent?" reconcile pass — is **advisory**.
...
scripts/wiki/
  ...
  wiki.test.ts           # engine regression suite (synthetic repos)
...
- **`.wiki/config.json`** — your wiki's `name` and the `highRisk` globs (contracts, routes, migrations, schema…) whose changes deserve the sharpest review.
